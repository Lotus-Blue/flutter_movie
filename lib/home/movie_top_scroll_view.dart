...
class _MovieTopScrollViewState extends State<MovieTopScrollView>
  ...
  Widget build(BuildContext context) {
    return MovieTopBannerView(banners);
    /*
    Container(
      color: Colors.white,
      child: Column(
  ...
  Future<void> fetchData() async {
    ...
    var top250Data = MovieDataUtil.getMovieList(
        await client.getTop250List(start: 0, count: 10));
    var usBoxData = MovieDataUtil.getMovieList(await client.getUsBoxList());
    var newMovieData =
        MovieDataUtil.getMovieList(await client.getNewMoviesList());
    PaletteGenerator paletteGenerator1 =
        await PaletteGenerator.fromImageProvider(
            CachedNetworkImageProvider(weeklyData[0].images.small));
    PaletteGenerator paletteGenerator2 =
        await PaletteGenerator.fromImageProvider(
            CachedNetworkImageProvider(top250Data[0].images.small));
    PaletteGenerator paletteGenerator3 =
        await PaletteGenerator.fromImageProvider(
            CachedNetworkImageProvider(newMovieData[0].images.small));
    PaletteGenerator paletteGenerator4 =
        await PaletteGenerator.fromImageProvider(
